...
export function buildSign(occasion, name) {
  return `Happy ${occasion}${name.length > 0 ? " " +name : "" }!`;
}
...
export function buildBirthdaySign(age) {
  return `${buildSign("Birthday", "")} What a ${age >0 && age <50 ? 'young' : 'mature'} fellow you are.`;
}
...
export function costOf(sign, currency) {
  ...
  return `Your sign costs ${Number.parseFloat(price).toFixed(2)} ${currency}.`;
}
